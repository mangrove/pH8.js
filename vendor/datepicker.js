...
Control.DatePicker.prototype = {
	initialize: function(element, options) {
		...
		if (this.options.icon) {
			...
			this.icon.alt = this.tr('Open calendar');
			...
			Element.insert(this.element, { after: this.icon });
	...
	show: function () {
		if (!this.pickerActive) {
			...
			//var pickerTop = /MSIE/.test(navigator.userAgent) ? pos[1] + 'px' : pos[1] + 'px';
			console.log(pickerTop);
...
Control.DatePicker.Language = {
    ...
    'nl': { 
        ...
        strings: { 
            ...
            'Select Date and Time': 'Datum en tijd kiezen',
	...
	drawCalendar: function(container, date) {
		...
		if (this.options.timePicker) {
			...
			if (this.options.timePickerAdjacent) {
				...
				var hr = new Element('hr',{ 'class': 'gray'});
				cell.appendChild(hr);
			}

            if (this.options.timePicker24h) {
                for (var j = 0; j < 4; ++j) {
                    row = timeTable.insertRow(rows++);
	                for (var i = 0; i < 6; ++i){
	                    ...
	                    cell.onclick = this.hourClickedListener((j*6)+i);
			...
			var hr_lg = new Element('hr',{ 'class': 'lightgray'});			
			cell.appendChild(hr_lg);
			cell = row.insertCell(1);
